...
def compute_enrichment(gene_set, target_genes, all_genes, label=None):
    ...
    return pd.Series(
        res,
        index=["pvalue","precision","recall","f1score","TP","target_set_size","gene_set_size"],name=label )
...
def compute_trn_enrichment(gene_set, all_genes, trn, max_regs=1, fdr=0.01, method="both", force=False):
    """
